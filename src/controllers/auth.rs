use crate::{
  ...
  models::{ _entities::users, users::{ LoginParams, RegisterParams } },
...
async fn verify(
  ...
  let Ok(user) = users::Model::find_by_verification_token(
    &ctx.db,
    &token
  ).await else {
...
async fn forgot(
  ...
  let Ok(user) = users::Model::find_by_email(
    &ctx.db,
    &params.email
  ).await else {
...
async fn reset(
  ...
  let Ok(user) = users::Model::find_by_reset_token(
    &ctx.db,
    &params.token
  ).await else {
    // we don't want to expose our users email. if the email is invalid we still
