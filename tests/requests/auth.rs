...
use loco_benchmark::{ app::App, models::users };
...
async fn can_login_with_verify(
  ...
  request::<App, _, _>(|request, ctx| async move {
    ...
    let email_verification_token = user.email_verification_token.expect(
    ...
    let user = users::Model
      ::find_by_email(&ctx.db, email).await
...
async fn login_with_un_existing_email() {
  ...
  request::<App, _, _>(|request, _ctx| async move {
    ...
    login_response.assert_json(
      &serde_json::json!({"error": "unauthorized", "description": "You do not have permission to access this resource"})
    );
  }).await;
}
...
async fn can_reset_password() {
  ...
  request::<App, _, _>(|request, ctx| async move {
    ...
    let forget_response = request
      .post("/api/auth/forgot")
      .json(&forgot_payload).await;
    assert_eq!(
    ...
    let user = users::Model
      ::find_by_email(&ctx.db, &login_data.user.email).await
      .expect("Failed to find user by email");
...
async fn can_get_current_user() {
  ...
  request::<App, _, _>(|request, ctx| async move {
    ...
    let response = request
      ...
      .add_header(auth_key, auth_value).await;
...
async fn can_auth_with_magic_link() {
  ...
  request::<App, _, _>(|request, ctx| async move {
    ...
    let user = users::Model
      ::find_by_email(&ctx.db, "user1@example.com").await
      .expect("User should be found");

    let magic_link_token = user.magic_link_token.expect(
      "Magic link token should be generated"
    );
    let magic_link_response = request.get(
      &format!("/api/auth/magic-link/{magic_link_token}")
    ).await;
    assert_eq!(
...
async fn can_reject_invalid_magic_link_token() {
  ...
  request::<App, _, _>(|request, ctx| async move {
    ...
    let magic_link_response = request.get(
      "/api/auth/magic-link/invalid-token"
    ).await;
    assert_eq!(
      magic_link_response.status_code(),
      401,
